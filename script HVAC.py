...
def splitPipeType(data):
  x = data.split("|")
  return x[3]

def splitPipeMaterial(data):
  x = data.split("|")
  return x[4]

def splitPipeSpeci(data):
  x = data.split("|")
  return x[5]
...
hvacPipeType = []
for data in rowDataList:
  hvacPipeType.append(splitPipeType(data))

hvacPipeMaterial = []
for data in rowDataList:
  hvacPipeMaterial.append(splitPipeMaterial(data))

hvacPipeSpeci = []
for data in rowDataList:
  hvacPipeSpeci.append(splitPipeSpeci(data))

ins = [[productName], [integrety], [insulation], [wallFloor], [constructionType], [subC], [thickness], [hvacPipeType], [hvacPipeMaterial], [hvacPipeSpeci]]
...
for i in range(429):
    if(i/10 < 1):
        id.append('H' + '0000' + str(i))
    elif(i/100 < 1):
        id.append('H' + '000' + str(i))
    elif(i/1000 < 1):
        id.append('H' + '00' + str(i))
    elif(i/10000 < 1):
        id.append('H' + '0' + str(i))
    else:
        id.append('H' +  str(i))
...
#   cur.execute(create_script)
  ...
  for i in range(429):
        insert_script = 'INSERT INTO hvac_t (hvac_id, hvac_pipe_type, hvac_pipe_material, hvac_pipe_speci) VALUES (%s, %s, %s, %s)'
        insert_value = [id[i], hvacPipeType[i], hvacPipeMaterial[i], hvacPipeSpeci[i]]
